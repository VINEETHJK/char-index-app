...
data = f.read()
data = str(data)
CHARACTERS = ["Kurtz", "manager", "Russian", "pilgrims", "cannibals", "aunt","brickmaker", "Accountant", "helmsman", "mistress", "intended", "Marlow", "Fresleven"]
...
def get_names(data, charaters=CHARACTERS):
    data = data.strip().split()
    punctuation = '.,?!"'
    data[:] = [word.strip(punctuation) for word in data]
    names=[]
    for word in data:
        if word in charaters:
            names.append(word)
